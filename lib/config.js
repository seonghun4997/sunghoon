...
export const DEFAULT_CONFIG = {
  ...
  texts: {
    ...
    level: "LV.25",
    bizTitle: "운영중인 사업 8개",
    bizDesc: "솔직히 관리 잘 안 되는 것도 많습니다. 매출·구조 등 상세 정보는 잘 공개 안 하는 성격입니다.",
...
export function mergeConfig(saved) {
  ...
  return {
    ...
    info: Array.isArray(saved.info) && saved.info.length > 0 ? saved.info : DEFAULT_CONFIG.info,
    stats: Array.isArray(saved.stats) && saved.stats.length > 0 ? saved.stats : DEFAULT_CONFIG.stats,
    biz: Array.isArray(saved.biz) && saved.biz.length > 0 ? saved.biz : DEFAULT_CONFIG.biz,
  };
}
