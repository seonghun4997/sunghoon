...
export async function readLatestConfig(client) {
  ...
  if (rows && rows.length) {
    try {
      return { id: rows[0].id, updated_at: rows[0].created_at, data: JSON.parse(rows[0].content) };
    } catch (e) {}
  }
  // 마이그레이션: 과거 site_config 저널(#100~) → 1번 행 순으로 시도
  try {
    const { data: j } = await client
      .from("site_config").select("id,data,updated_at").gte("id", 100)
      .order("id", { ascending: false }).limit(1);
    if (j && j.length) return { id: "old-" + j[0].id, updated_at: j[0].updated_at, data: j[0].data };
    const { data: legacy } = await client
      .from("site_config").select("data,updated_at").eq("id", 1).maybeSingle();
    if (legacy) return { id: "old-1", updated_at: legacy.updated_at, data: legacy.data };
  } catch (e) {}
  return null;
}
...
export async function writeNewConfig(client, clean) {
  const { data: ins, error } = await client
    .from("patchnotes")
    .insert({ version: CONFIG_VER, content: JSON.stringify(clean) })
  ...
  return { check: { id: check?.id, updated_at: check?.created_at, data: parsed } };
}
